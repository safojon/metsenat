...
const sponsor = ref(props.sponsor.sponsor);
const summa = ref(props.sponsor.summa);
const Sponsors = ref({});;
const Result = ref({});
...
const Add = async () => {
  if (summa.value) {
    ...
    try {
      ...
      const response = await api.put(`/sponsor-summa-update/${props.data.id}/`, data);
...
const Del = async () => {
    emit('ClouseEdit');
    try {
      const response = await api.delete(`sponsor-summa-delete/${props.data.id}/`);
      Result.value = response.data;
    } catch (error) {
      console.log(error.response?.data || error.message);
    }
  ...
  <div class="w-full fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50">
    <div class="w-[586px] h-auto bg-white rounded-[12px] flex flex-col p-[28px]">
      ...
      <div class="flex items-center justify-between">
        <div class="font-bold text-[24px] text-[rgba(40,41,61,1)]">Homiylarni tahrirlash</div>
      ...
      <div class="mt-[28px]">
        ...
        <div class="flex justify-end mt-[28px]">
          <div class="flex gap-[16px]">
            <button @click="Del"
            class="w-[209px] h-[42px] bg-[rgba(255,236,235,1)] rounded-[5px] text-[rgba(255,73,69,1)] font-medium text-[14px] flex items-center justify-center"><span
              class="icon-trash text-[20px] pr-[10px]"></span> Homiyni o'chirish</button>
            <button @click="Add"
              class="w-[150px] h-[42px] bg-[rgba(51,102,255,1)] rounded-[5px] text-white font-medium text-[14px] flex items-center justify-center"><span
                class="icon-save text-[15px] pr-[10px]"></span> Saqlash</button>
          </div>
        </div>
      </div>
    </div>
